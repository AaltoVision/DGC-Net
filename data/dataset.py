...
class HomoAffTpsDataset(Dataset):
    ...
    def get_grid(self, H, ccrop):
        ...
        X,Y = np.meshgrid(np.linspace(0, W_FULL-1, W_FULL),np.linspace(0, H_FULL-1, H_FULL))
        X_,Y_ = X,Y
        X,Y = X.flatten(), Y.flatten()
        ...
        Xwarp_crop = X_grid_pivot[Y_CCROP:Y_CCROP+H_SCALE, X_CCROP:X_CCROP+W_SCALE]
        Ywarp_crop = Y_grid_pivot[Y_CCROP:Y_CCROP+H_SCALE, X_CCROP:X_CCROP+W_SCALE]
        X_crop = X_[Y_CCROP:Y_CCROP+H_SCALE, X_CCROP:X_CCROP+W_SCALE]
        Y_crop = Y_[Y_CCROP:Y_CCROP+H_SCALE, X_CCROP:X_CCROP+W_SCALE]

        # crop grid
        Xwarp_crop_range = 2 * (Xwarp_crop - X_crop.min()) / (X_crop.max() - X_crop.min()) - 1
        Ywarp_crop_range = 2 * (Ywarp_crop - Y_crop.min()) / (Y_crop.max() - Y_crop.min()) - 1
        grid_crop = torch.stack([Xwarp_crop_range, Ywarp_crop_range], dim=-1)
    ...
    def __getitem__(self, idx):
        ...
        if ((transform_type == 0) or (transform_type == 1)): # affine transformation
            ...
            if (transform_type == 0):
                ...
                theta = torch.Tensor(theta.astype(np.float32)).expand(1, 18, 1, 1)
        ...
        if self.transforms is not None:
            cropped_source_image = self.transforms(cropped_source_image.transpose(0,1).transpose(1,2).numpy().astype(dtype=np.uint8))
            cropped_target_image = self.transforms(cropped_target_image.transpose(0,1).transpose(1,2).numpy().astype(dtype=np.uint8))
        ...
        if (transform_type == 0):
            ...
            for layer_size in self.pyramid_param:
                ...
                #mask_x.append(mask[:,:,0].long())
                #mask_y.append(mask[:,:,1].long())
                mask_x.append(mask[:,:,0])
                mask_y.append(mask[:,:,1])
        ...
        return {'source_image': cropped_source_image, 'target_image': cropped_target_image, \
...
class TpsGridGen(Module):
    ...
    """
    def __init__(self, out_h=240, out_w=240, use_regular_grid=True, grid_size=3, reg_factor=0, use_cuda=False):
